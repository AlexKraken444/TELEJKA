 ...
 if(path[0]==='polls'){
  if(path.length===1){const chat=req.nextUrl.searchParams.get('chat');if(chat){z.uuid().parse(chat);await canSendChat(chat,userId);}
   if(req.method==='GET'){const author=req.nextUrl.searchParams.get('author');if(author)z.uuid().parse(author);
    return json(await sql`SELECT p.*,telejka_user(p.owner_id,${userId}::uuid) author,COALESCE((SELECT jsonb_object_agg(v.choice,v.n) FROM (SELECT choice,count(*)::int n FROM poll_votes WHERE poll_id=p.id GROUP BY choice)v),'{}') counts,(SELECT choice FROM poll_votes WHERE poll_id=p.id AND user_id=${userId}) choice FROM polls p WHERE p.chat_id IS NOT DISTINCT FROM ${chat}::uuid AND (${chat}::uuid IS NOT NULL OR telejka_can_view(p.owner_id,${userId}::uuid)) AND (${chat}::uuid IS NULL OR EXISTS(SELECT 1 FROM members mb JOIN messages msg ON msg.id=p.message_id WHERE mb.conversation_id=p.chat_id AND mb.user_id=${userId} AND (mb.cleared_at IS NULL OR msg.created_at>mb.cleared_at))) AND (${author}::uuid IS NULL OR p.owner_id=${author}::uuid) ORDER BY p.created_at DESC LIMIT 50`);}
   if(req.method==='POST'){const parsed=chat?envelope.parse(input.payload):payload.parse(input.payload);const n=chat?z.number().int().min(2).max(10).parse(input.optionCount):payload.parse(parsed).options.length;
    const row=await sql.begin(async tx=>{const [poll]=await tx`INSERT INTO polls(owner_id,chat_id,payload,option_count) VALUES(${userId},${chat},${tx.json(parsed)},${n}) RETURNING id`;if(chat){await tx`INSERT INTO messages(id,conversation_id,user_id,body) VALUES(${poll.id},${chat},${userId},'Опрос')`;await tx`UPDATE polls SET message_id=${poll.id} WHERE id=${poll.id}`;}else{await tx`INSERT INTO posts(id,user_id,body) VALUES(${poll.id},${userId},${payload.parse(parsed).question})`;await tx`UPDATE polls SET post_id=${poll.id} WHERE id=${poll.id}`;}return poll});if(chat)scheduleMessagePush(row.id);return json(row);}
  }
  if(path.length===2&&(req.method==='POST'||req.method==='GET')){const pid=z.uuid().parse(path[1]);const [poll]=await sql`SELECT * FROM polls WHERE id=${pid}`;if(!poll)throw new FeatureError(404,'Опрос не найден.');if(poll.chat_id)await canSendChat(poll.chat_id,userId);else await canContact(userId,poll.owner_id);if(poll.chat_id){const [visible]=await sql`SELECT 1 FROM members m JOIN messages msg ON msg.id=${poll.message_id} WHERE m.conversation_id=${poll.chat_id} AND m.user_id=${userId} AND (m.cleared_at IS NULL OR msg.created_at>m.cleared_at)`;if(!visible)throw new FeatureError(404,'Опрос недоступен.');}if(req.method==='GET'){const [result]=await sql`SELECT p.*,telejka_user(p.owner_id,${userId}::uuid) author,COALESCE((SELECT jsonb_object_agg(v.choice,v.n) FROM (SELECT choice,count(*)::int n FROM poll_votes WHERE poll_id=p.id GROUP BY choice)v),'{}') counts,(SELECT choice FROM poll_votes WHERE poll_id=p.id AND user_id=${userId}) choice FROM polls p WHERE id=${pid}`;return json(result);}const choice=z.number().int().min(0).max(poll.option_count-1).parse(input.choice);await sql`INSERT INTO poll_votes VALUES(${pid},${userId},${choice}) ON CONFLICT(poll_id,user_id) DO UPDATE SET choice=EXCLUDED.choice`;return json({ok:true});}
 }
